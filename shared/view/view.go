...
func (v *View) RenderSingle(w http.ResponseWriter) {
	...
	for i, name := range templateList {
		...
		if err != nil {
			http.Error(w, "RenderSingle: Template Path Error: "+err.Error(), http.StatusInternalServerError)
			return
		}
		templateList[i] = path
	}
	...
	if err != nil {
		http.Error(w, "RenderSingle: Template Parse Error: "+err.Error(), http.StatusInternalServerError)
		return
	}
	...
	if err != nil {
		http.Error(w, "RenderSingle: Template File Error: "+err.Error(), http.StatusInternalServerError)
	}
}
...
func (v *View) Render(w http.ResponseWriter) {
	...
	if !ok || !viewInfo.Caching {
		...
		for i, name := range templateList {
			...
			path, err := filepath.Abs(v.Folder + string(os.PathSeparator) + name + "." + v.Extension)
			if err != nil {
				// this reveals too much info about the server:
				// http.Error(w, "Render: template path error: "+err.Error(), http.StatusInternalServerError)
				http.Error(w, "Render: template path error: ", http.StatusInternalServerError)
				return
			}
			templateList[i] = path
		}
		...
		if err != nil {
			...
			http.Error(w, "Render: template parse error!", http.StatusInternalServerError)
			return
		}
	...
	if err != nil {
		http.Error(w, "Render: Template File Error: "+err.Error(), http.StatusInternalServerError)
	}
}
...
func (v *View) SendFlashes(w http.ResponseWriter) {
	...
	if err != nil {
		http.Error(w, "SendFlashes: JSON Error: "+err.Error(), http.StatusInternalServerError)
		return
	}
